 ...
 * exit mmWM if memory allocation fails
 */
static client *create_client(xcb_window_t win, xcb_atom_t wtype)
{
...
void setmaximize(client *c, bool maximize)
{
    ...
    if (maximize) {
        xcb_move_resize(dis, c->win, M_GAPS, M_WY + M_GAPS + PANEL_HEIGHT,
...
int main(int argc, char *argv[])
{
    ...
    if (argc == 2 && argv[1][0] == '-') switch (argv[1][1]) {
        case 'v':
            errx(EXIT_SUCCESS,
            "mmWM - by kaugm (a million thanks to sulami and FrankenWM)");
        case 'h':
